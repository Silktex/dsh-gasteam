...
- [ ] Add structured checkpoints containing the assignment, workflow position, artifact references, and next action. The store and DSH prompt preserve structured task/artifact/action context, but coordinator admission still writes generic implement/repair steps without the workflow ID or exact workflow step; wire that immutable workflow position before closing this requirement.
...
- [ ] Add one idempotent submit operation that records task/attempt identity, source commit, result evidence, and verification policy revision, then queues integration.
- [ ] Serialize the queue by canonical repository and target branch across teams. Preserve expected-target checks and clean-checkout requirements.
...
- [ ] Define validated, versioned workflow templates with parameters, step dependencies, acceptance requirements, retry policy, and artifact references. Choose one file format and document it.
- [ ] Persist an immutable workflow definition/version for each execution so editing a template does not alter in-flight obligations.
- [ ] Implement checkpointed execution and fresh-session handoff with task evidence, pending steps, and unresolved blockers restored.
- [ ] Ship implementation/test/review/integration and investigation/report templates. Include a release template with an explicit externally authorized publication gate.
- [ ] Add workflow create/inspect/resume controls and clear diagnostics for missing parameters or invalid graphs.
...
- [ ] Expose project register/inspect/pause operations with repository-specific policy and capacity. Persist coordinator identity independently of browser chats.
- [ ] Extend batches to reference globally identified work across projects and teams. Detect cycles across project dependencies.
- [ ] Derive batch progress from required acceptance states, retaining reopened and failed histories. Show ready work without an active assignment as an actionable condition.
- [ ] Persist completion subscriptions and deduplicated notification delivery. Reopening and later completion must have explicit notification semantics.
- [ ] Support a coordinator planning interaction that creates and dispatches a multi-project batch through the same validated services.
...
| M1 Durable records | in_progress | Project + assignment suites: 15 tests pass; three-process worker/crash replay passes. Records, checkpoints, internal concurrent capacity and journal exclusion implemented. Coordinator assignment-to-worker wiring now admits configured DSH workers; durable runtime policy and specialized acceptance operations remain. Team spawn/wakeup enforce concurrent capacity independently of retained history. |
...
| M4 Integration lifecycle | in_progress | Host pinned integration admission preserves stable ID/source/policy and deduplicates replay. Durable host submissions bind attempts, source/evidence and policy revision and replay pending admission. Automatic submission and owned integration execution now pass real-Git success/failure scenarios. Verified task acceptance and a real-Git diamond DAG now pass. Canonical cross-Team/process execution exclusion is implemented. Bounded stale-target re-verification and fresh-process retry replay now pass. Bounded explicit verification repair, pinned budgets and reserved-repair process replay now pass. The host-only opt-in review gate binds source/target/candidate/gate/review ID, and stale retries clear the current receipt while retaining historical receipt evidence. Explicit audited non-code acceptance and intent/receipt SIGKILL replay now pass. Candidate retention scheduling is implemented with pause and conservative uncertain-outcome handling; global backlog policy and broader crash coverage remain. |
| M5 Workflows/handoff | in_progress | Strict immutable JSON workflow definitions, checkpoint/receipt store, retry backoff and explicit publication authorization core implemented. The initial investigation/report vertical now pins template/version, binds host-created tasks atomically, advances from accepted report receipts, resumes after restart, and exposes scoped controls; The code workflow now requires an explicit candidate-bound review before verified integration, preserves repair/source history, and passes fresh-process approved/rejected/repair cases; a configured server-only release runtime now passes pause, authority/config binding, accepted preparation, exact receipt, and bounded failure tests; real publisher conformance and broader handoff remain unfinished. |
...
| M7 Multi-project batches | in_progress | Durable batch-state foundation covers global refs, accepted-state progress, history, and completion epochs; review fixes for concurrent intent admission, aggregate dependency cycles, observation ordering, and durable bounds pass isolated tests. Coordinator/planning/RPC/UI wiring and two-repository acceptance remain unfinished. |
| M8 Dashboard | pending | — |
| M9 External providers | in_progress | Capability/CLI adapter, durable attempt store, compiled PID-namespace supervisor, idempotent launch claims, and stop/spool proof validated. Combined checkpoint passes 306 regular tests and 17 process scenarios. DSH adaptation, provider admission/routing, binary verification, and real authenticated assignment conformance remain unfinished. |
...
- Current milestone and acceptance slice: M5 authorized publication runtime, M9 verified CLI admission/provider routing, and M7 cross-project batch service; drafts remain under implementation/review.
- Last completed slice: authorized release runtime with accepted manifest preparation, pinned host authority/receipts and bounded failure behavior; durable cross-project batch-state foundation; verified, bounded CLI admission. Combined clean archive passes 323 regular tests and 17 process scenarios. Coordinator batch/provider wiring remains unfinished.
...
- Green evidence and exact commands: clean combined archive `/tmp/gasteam-m5m9-combined-pizZFv` passed frozen install, build, types, docs, 306 regular tests, 17 process scenarios, and smoke. Published `2c2d76cfec83bbbb209bfa8c8ea9a026caf49bf9` passed the same exact-commit CI, run `33989542670`; local receipt `/tmp/gasteam-ci-2c2d76c.log`.
- Changed files and commit: published `2c2d76cfec83bbbb209bfa8c8ea9a026caf49bf9` includes code workflow `7d649fe` and external-runtime foundation `9ea18de`. Publication workflow, Codex admission, and cross-project batch drafts are separate uncommitted work. Preserve local .agents/, skills-lock.json and pending.md.
...
- Next concrete action: close publication review findings (authority, receipt/config binding, accepted preparation, bounded failures), validate CLI admission and route an external assignment, then integrate cross-project batches. Continue all remaining M1–M11 and final acceptance requirements without narrowing scope.
- Release actions still required: remaining M1 items, M2–M11, all final acceptance scenarios, applicable publication/installation verification. Local service was inspected only; no restart or durable-user-data modification occurred.
- Previous goal-turn classification: progress — three verified commits published, exact-SHA CI succeeded with 306 regular tests and 17 process scenarios; subsequent drafts received concrete independent review.
