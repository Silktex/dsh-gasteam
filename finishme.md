...
| M7 Multi-project batches | in_progress | Exact-operator planning, global dependency dispatch, legacy-compatible revision tuples, acceptance-derived status, bounded model inspection, and durable in-app notification replay pass. A two-repository code batch survives SIGKILL with exact first assignment identity and one dependent assignment, then deduplicates notification on another restart. Actual failing-verification-A/healthy-B integration acceptance and the separately developed workspace batch RPC surface remain open. |
...
- Last completed slice: M7 coordinator/model planning and two-repository process restart, M5 inspect diagnostics and requirement audit, plus deterministic inherited-pipe admission regression. Clean base383 archive passes344 regular tests (one optional skip),18 process scenarios, build/types/docs/smoke.
...
- Next concrete action: publish reviewed M7/M5 checkpoint; finish provider routing/recovery and real conformance, workspace batch RPC plus failed-project independence acceptance, M6 recovery vertical, and full M8 dashboard. Continue every remaining milestone and final acceptance requirement.
