...
- [ ] Introduce execution attempts distinct from worker identity and root-session identity. Preserve immutable attempt history and current ownership.
- [ ] Specify and implement legal assignment/attempt transitions, revision checks, terminal evidence, and reconciliation records. Persist intent before creating externally visible work.
- [ ] Add structured checkpoints containing the assignment, workflow position, artifact references, and next action. Ensure workers receive reconstructable context.
...
- Current milestone and acceptance slice: M1/M2 assignment-to-worker lifecycle integration and M3 scheduling; M0 complete, M2 discovery/pause acceptance passes.
- Last completed slice: the initial investigation/report workflow runtime binds immutable workflow steps to atomic host-created Team tasks, uses accepted report receipts as its only progress input, and resumes the next step after a fresh-process crash without duplicating the task or worker. Code and publication workflow runtime paths remain unfinished.
...
- Green evidence and exact commands: `pnpm build`, `pnpm typecheck`, `pnpm test` (29 files, 270 tests including seven independent health tests; workflow checkpoint subset: 28 files, 263 tests), `pnpm test:acceptance` (14 built-process scenarios), and `pnpm test:smoke` passed. See `docs/completion-evidence.md`.
- Changed files and commit: published checkpoint HEAD `854591fa4c7b894c23a87201a4661a8d6206534d` passed exact-SHA GitHub CI and independent clean install. Later non-code/workflow-core/cleanup changes are uncommitted. Preserve local .agents/, skills-lock.json and pending.md.
...
- External blockers: none established. External CLI authentication and real-runtime acceptance have not yet been inspected.
- Next concrete action: expand the workflow runtime beyond report-review to code/integration and authorized publication templates. Candidate retention is wired behind explicit `execution.candidateRetention`; target-movement retry policy is still fixed. Expand integration backlog policy, recovery/escalation, remaining workflow, multi-project, UI, external-runtime and release milestones without reducing M1–M11 scope.
...
- Previous goal-turn classification: progress — delegated non-code acceptance, workflow core and cleanup primitive implemented/reviewed; 247 regular tests and thirteen process scenarios validated.
...
- [ ] Add a `docs/README.md` documentation index and link it from `readme.md`. Cross-link the usage, installation, and debug pages with `docs/autonomous-architecture.md` and `docs/completion-evidence.md`; remove duplicated procedural text where a canonical page is clearer without weakening the top-level quick start.
- [ ] Validate every documented command against a clean standalone installation and the shipped CLI/profile. Add a documentation link/command check suitable for CI, record the exact validation evidence in `docs/completion-evidence.md`, and keep these tasks open wherever the corresponding product surface or real-runtime acceptance remains incomplete.
