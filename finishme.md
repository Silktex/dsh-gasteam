...
- [x] Persist dispatch requests with priority and stable ordering. Schedule only eligible tasks; explain why blocked tasks cannot run.
- [x] Enforce global and per-project active capacity, dispatch pacing, and bounded retry/backoff. Define fairness so one busy project cannot starve another.
- [x] Atomically reserve assignment/capacity before provisioning. Reconcile failed provisioning without leaking a slot or erasing failure history.
- [x] Support pause/resume and cancellation with defined treatment of queued versus active work. Expose read-only dispatch status through tools and RPC.
...
- [x] Distinguish progressing, idle, waiting on a dependency, awaiting operator input, stale, unavailable, and failed attempts. Avoid treating all unchanged event counts as a stuck worker.
- [x] Define progress signals and configurable deadlines for DSH and external providers. Combine authoritative runtime state with durable work state.
- [x] Apply bounded recovery stages: prompt/nudge where appropriate, checkpointed handoff, fenced replacement, then escalation. Preserve ownership until replacement is safe.
- [x] Persist severity, source, diagnostics, affected work, acknowledgement, resolution, and bounded re-escalation state.
- [x] Provide a durable in-app operator inbox. External notification transports must be configurable and explicitly enabled; do not send test messages to real people.
...
- [x] Show projects, workers/attempts, assignments, workflow steps, batches, merge queues, and escalations in one workspace view.
- [x] Show actionable reasons for blocked work, queue age, capacity usage, last progress, retries, and relevant verification diagnostics. Add usage/cost data only when the provider reports it; unknown must remain unknown.
- [x] Add pause/resume, safe cancellation, retry, reassignment, handoff, and escalation acknowledgement using typed service operations and revision checks.
- [x] Provide an activity feed with durable event references and reconnect behavior. Paginate/bound history so long-running projects do not require loading every attempt.
- [x] Update model tools, RPC descriptors/codecs, English/Chinese copy, keyboard access, and browser error states with each operation.
...
- [x] Define provider capabilities for start, resume or fresh-session handoff, status, cancellation, messaging, artifact submission, and optional usage. Advertise unsupported operations explicitly.
- [x] Adapt the current in-process DSH implementation to that interface without changing its existing guarantees.
- [x] Implement at least one external CLI provider selected from the authenticated runtimes actually available on the host. Record the selected runtime/version and verify its official documentation at implementation time.
- [x] Route provider choice by project or task policy. Preserve existing permissions, cwd isolation, identity fencing, and durable admission semantics.
- [x] Classify startup/authentication errors and unsupported resume behavior. Terminate owned process trees on cancellation and teardown; do not claim process death from missing output alone.
...
- [x] Update `readme.md` with installation, project setup, unattended operation, workflow examples, recovery, runtime providers, troubleshooting, retention, and uninstall/rollback instructions.
- [x] Provide a validated autonomous profile/example enabling the required worktree, scheduler, coordinator, integration, and supervisor components together. Show required target branch and verification configuration explicitly.
- [x] Add fresh-clone CI covering frozen install, build, types, host/client tests, deterministic restart acceptance, and installed CLI smoke. Add suitable scripts for the new acceptance suites.
...
| M3 Scheduler | done | Durable ordering/fairness/capacity/pacing, bounded runtime and provisioning retries, positive stop ownership, pause/cancel/status controls and verified dependencies are covered by scheduler/assignment/coordinator tests. Explicit failed-prerequisite and scheduling-tool transcripts complete the audit; combined178 tests and21 process scenarios pass. See completion evidence. |
...
| M6 Health/recovery | done | Classified durable health, bounded nudge and checkpointed replacement budgets, ownership fences, retained worktrees, and acknowledged/resolved/re-escalated operator flow are tested. External transports are absent and disabled. |
...
- Last completed slice: concurrent external terminal observation made idempotent with deterministic stale-observer coverage and exact corrective CI.
- Red evidence: CI19cf882 exposed lost fast-launch stderr; CI80d6001 exposed birth-identity capture after child reaping; CI9f3c5fe exposed the exited-child cancellation receipt race; CI3c081ec exposed concurrent terminal-versus-uncertainty publication. Deterministic regressions and corrections are recorded in completion evidence.
- Green evidence and exact commands: final corrective commit b78acfc passed exact CI34009813442. It ran frozen install, docs, build, types, 509 regular host/client tests with two optional skips, 21 process acceptance scenarios, installed CLI smoke, and independent packed release/legacy upgrade validation.
- Changed files and commit: implementation and all corrective publication are complete through b78acfcfa394c31fa03f5f152d087ab0e1057220. Unrelated docs/README.md, darkfactory.md and docs/worklist.md remain untouched.
...
- Release actions still required: none for the implemented release. Final corrective commit `b78acfcfa394c31fa03f5f152d087ab0e1057220` passed exact CI34009813442. Production installation/restart and future public registry publication were not requested. Disposable validation does not claim user-service migration; the profile incident and narrow correction remain documented.
...
- [x] Create `docs/usage.md` with detailed operator guidance for registering a project, configuring repository and verification policy, submitting dependent tasks and batches, running unattended coordination, monitoring assignments and integration, pausing/resuming/cancelling work, accepting non-code results, and using workflow and runtime-provider controls. Include complete examples, expected state transitions, and links to the relevant CLI, model-tool, RPC, and dashboard surfaces that actually ship.
- [x] Create `docs/installation.md` with prerequisites, supported runtime and package-manager versions, fresh-clone and published-package installation, build/profile configuration, autonomous service startup, required repository permissions, upgrade and durable-data migration, health verification, backup, rollback, and uninstall steps. Clearly separate development-checkout instructions from standalone installation and ensure no instructions depend on the former full DeepSeek Harness source checkout.
...
- [x] Create `docs/gastown-comparison.md` comparing Gas Town and all its features vs this plugin, listing all similarities, differences, and features not available in the plugin.
- [x] Validate every documented command against a clean standalone installation and the shipped CLI/profile. The link/shell checker is complemented by executed independent packed installs, CLI worker/promotion, deterministic restart, profile install/removal, and disposable backup/restore. Authenticated external receipts are recorded separately; production service operations and unavailable registry installation are conditional procedures, not executed claims. Record the exact validation evidence in `docs/completion-evidence.md`, and keep this task open wherever the corresponding product surface or real-runtime acceptance remains incomplete.
