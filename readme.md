...
### Autonomous operation development

Unattended operation is being implemented under [finishme.md](finishme.md). `pnpm test:acceptance` runs a separate fresh-process gate after `pnpm build`. The startup-discovery scenario now passes: it registers a project, accepts a task, waits for a durable-write IPC acknowledgement, kills the writer, and starts the built coordinator plugin with no browser or pre-opened Lead. The new process finds the task and retains its coordinator identity. The gate also proves that configured execution starts one accepted task automatically in an isolated worktree after restart. It also completes a persisted diamond DAG on the coordinator timer and recovers after crashes before the merged record, after promotion, and after the task receipt. The gate also resumes a killed in-process worker with its uncommitted edits and completes the dependent DAG. The full final acceptance matrix remains unfinished.
...
Coordinator health is also opt-in observation. Configure `execution.health` with `dshDeadlineMs`, `externalDeadlineMs`, `escalationCooldownMs`, and `maxEscalationsPerCondition` to record generation-fenced attempt observations and expose a registered-Lead escalation inbox. Live DSH operations remain execution-unknown; health does not infer a stuck tool or trigger a nudge, stop, handoff, or replacement. Dependency/report waits and failed integration become actionable incidents, while intentional cancellation is not critical. Acknowledgement is revision- and policy-fenced, and accepted receipts clear only the exact generation while retaining history. Provider probes, recovery actions, external notifications, and dashboard health controls remain unfinished.

Enable `@deepseek-ai/dsh-experimental-tool-agent-team/coordinator` after the coordinator plugin to expose `team_dispatch_status`, `team_dispatch_pause`, `team_dispatch_priority`, and `team_dispatch_cancel` in Lead scopes. Calls require a registered project grant; pause uses the project control revision and priority uses the dispatch request revision. Browser Remote clients can call `agentTeams.scheduling` and `agentTeams.controlScheduling` with the same Lead identity. These interfaces return project-scoped status and reject cross-project access and stale revisions.
...
Interrupted in-process attempts retain assignment capacity and use at most three recovery rounds with persisted 1-, 2-, and 4-second delays. Recovery reuses the existing runtime/worktree and sends checkpoint context through a stable mailbox identity. Pending delivery is retried under the same identity; uncertain delivery does not release capacity. Exhausted interrupted rounds produce a stopped attempt requiring recovery. Configurable recovery policy, richer escalation/operator controls, and external-runtime recovery remain unfinished.
