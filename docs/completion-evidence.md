...
Checkpoint `185286b937aebab6d5f253925ec9fdb00673899a` subsequently passed [exact-commit GitHub CI](https://github.com/Silktex/dsh-gasteam/actions/runs/33994879702). Receipt: `/tmp/gasteam-ci-185286b.json`; watch log: `/tmp/gasteam-ci-185286b.log`.


## 2026-09-05 — Authoritative DSH health observation

The DSH observer requires exact live Lead, roster attempt identity and parent session. It records only in-process `tools/execute` dispatch handles after pre-execution policy approval; a durable tool request or restored session history alone remains unknown. Dispatch tokens independently retain nested operation lifetimes, with finally cleanup after errors. The coordinator health scan uses this evidence without adding recovery effects or weakening external fresh-identity probes.

Isolated archive `/tmp/gasteam-m6-route-observer-TsaET6` contains the published route plus this three-file slice. Build/types, 367 regular tests (two explicit skips), 18 process scenarios, docs and smoke pass. A subsequent test-only nested-dispatch regression passes all six focused observer tests: child completion preserves the active outer call and outer failure clears its handle. Manifest `combined-route-m6-overlay.sha256`; logs remain in the archive. Configurable durable nudge/handoff/replacement and full M6 operator resolution remain unfinished.

Checkpoint `cf56d86998da14e621849b796ce5df8505dea84f` passed [exact-commit GitHub CI](https://github.com/Silktex/dsh-gasteam/actions/runs/33995091711). Receipt `/tmp/gasteam-ci-cf56d86.json`.


## 2026-09-05 — Workspace batch RPC and independent failure recovery

Typed browser-safe plan/inspect/subscribe/inbox/acknowledgement routes use server-owned workspace-operator authority. Batch observations bind reports/submissions to the current attempt/generation, preserve active ownership alongside failed verification, and order terminal integration receipts independently from task completion. Legacy accepted receipt upgrades do not duplicate completion epochs; failed generation-one history survives active generation-two repair and acceptance.

Real compiled-process acceptance kills the coordinator after project A verification fails, then proves A's durable failure history while independent project B integrates `healthy-b.txt` with exactly one assignment. The route-plus-M7 snapshot passes 365 regular tests (two explicit skips), 19 process scenarios, build/types/docs/smoke. Final immutable archive `/tmp/gasteam-m7-rpc-failure-cf56d86` combines the unchanged M7 overlay with published cf56d86 and passes frozen install, build, typecheck and 57 focused batch/coordinator/Remote/built-library tests. Manifest `m7-rpc-failure-overlay.sha256`; logs `/tmp/gasteam-m7-cf56-{build,typecheck,focused}.log` and `/tmp/gasteam-m7-route-rpc-failure-*`. Full workspace dashboard and remaining milestone/final acceptance obligations are still open.

Checkpoint `03a692091cb388d25c1c65ff5ff603d8d12e8fcc` passed [exact-commit GitHub CI](https://github.com/Silktex/dsh-gasteam/actions/runs/33995336253). Receipt `/tmp/gasteam-ci-03a6920.json`.


## 2026-09-05 — M7 requirement audit

Published `03a6920` satisfies M7's five explicit requirements. Project registration/inspection/pause and independent repository policy/capacity use the coordinator/catalog authority. `coordinator-batches.spec.ts` covers global references and cross-batch union cycles, acceptance/history, actionable undispatched work and notification epochs/replay. `coordinator.spec.ts` covers authorized planning, cross-project dependency blocking, restored Leads, current-attempt receipt derivation and operator inbox/ack. Typed Remote codecs and permissions have direct coverage.

`tests/cold-start.acceptance.ts` two-repository scenarios fork plain Node through `tests/support/process-fixture.ts` with isolated HOME/DSH_HOME and no credentials. `restart-team.mjs` loads built plugins and installed published DSH dependencies, persists real JSONL, and verifies actual Git artifacts. SIGKILL after A integrates preserves its one assignment while B resumes; a separate scenario retains A's verification failure while independent B integrates once. Assertions provide the cross-project status transcript and durable notification replay/ack evidence. Project-specific configuration and status examples are in `docs/usage.md`. These are keyless installed-runtime smoke behavior; consumer tarball/registry installation is a separate unfinished M11 release gate. M7 is marked done; remaining milestones and final acceptance are unchanged.


## 2026-09-05 — Bounded Team teardown with retained drain ownership

Team disposal serializes close attempts, observes whether admitted operations settled, and retains unresolved ownership after a deadline. Child drains are shared across overlapping child sets, so partial provider cleanup cannot repeat cancellation. Later closes also await drains absent from the live roster, including hidden root A while root B still has visible children. Timeout alone never proves termination. Individual provider operations are deadline-bounded; a single global wall-clock deadline is not claimed.

Final isolated archive `/tmp/gasteam-m2-disposal-4ac4f1c` contains four reviewed files. Its authoritative manifest is **`m2-disposal-overlay-files.sha256`**; the similarly named `m2-disposal-overlay.sha256` is obsolete. Final build/types, Team91, regular376 with two explicit skips, process acceptance19, docs and smoke pass, logged at `/tmp/gasteam-m2-final-{build,typecheck,team,regular,acceptance,docs,smoke}.log`. A publication command incorrectly continued after rejecting the obsolete manifest and pushed only the already validated lifecycle return-type change as `000d920`. This follow-up publishes the remaining exact validated blobs after checking every hash before staging. No history was rewritten. M2's remaining audit and broader final acceptance remain open.
...
## 2026-09-05 — External code worktree and verified integration

Explicit project-scoped code opt-in provisions a provider-owned checkout without manufacturing a DSH Team member. Immutable intent precedes Git; a separate fsynced completion receipt follows actual Git close and verified checkout identity. Intent-only recovery, preexisting matching checkouts, swapped receipt/path/common-directory bindings and uncertain Git close remain blockers. In-process retry fencing and a fresh-process SIGKILL/live-Git regression prevent duplicate provisioning or premature CLI launch. Git output/deadlines are bounded and uncertain live handles remain owned.

The actual coordinator fixture runs the controlled external CLI through its isolated checkout, terminal output, pinned source submission, explicit external integration capability, real verification/merge and task acceptance. Ordinary Team member validation remains separate; external IDs are never used to stop a DSH session. Authenticated model conformance and the remaining complete M9 provider contract remain unfinished.

Final combined archive `/tmp/gasteam-external-code-1b209-final-FQn4dE` is based on published1b209a3, preserving the dashboard. Single authoritative `external-code-1b209-final.sha256` covers17 paths; all hashes verified before staging. Combined frozen install/build/types, compiled focused23 and docs pass (`/tmp/gasteam-external-code-1b209-*.log`). The unchanged external behavior on49cee passes full regular383 (two explicit skips), process acceptance19, docs and smoke (`/tmp/gasteam-external-code-49cee-*.log`). No authenticated model result is claimed.

Publication correction: the refreshed external manifest was accidentally reduced to `docs/usage.md`, so `65e3278` contains the configuration/evidence documentation only. The implementation is published separately from the exact expected16 source/test paths. Its authoritative manifest is `external-code-1b209-final-17paths.sha256` in the same final archive; root verified the complete17-path set, every hash, and the expected16-file staged source delta before committing. The previous docs-only manifest is invalid. No history was rewritten.

Dashboard checkpoint `1b209a37bb0262da2bc284a13518b542d91bb339` passed [exact-commit GitHub CI](https://github.com/Silktex/dsh-gasteam/actions/runs/33997140134); receipt `/tmp/gasteam-ci-1b209a3.json`. Comparison checkpoint `9e06fc9` completes the added date-pinned capability comparison after preserving the original draft at `/tmp/gasteam-comparison-before-audit.md`, reviewing authoritative upstream sources and correcting unsupported parity/completion claims. Documentation checks pass.

External-code implementation checkpoint `d73f580f27fb4b5294f06790bcd63b2cb0ab84af` passed [exact-commit GitHub CI](https://github.com/Silktex/dsh-gasteam/actions/runs/33997312194). Receipt `/tmp/gasteam-ci-d73f580.json`; log `/tmp/gasteam-ci-d73f580.log`. This confirms controlled-provider integration; authenticated model conformance remains separate.


## 2026-09-05 — Opt-in durable stale-operation nudge

M6 now has a bounded DSH nudge stage, configured by `execution.health.recovery.maxNudges`. The journal pins generation budget and message identity; AssignmentStore replays the same reservation without spending another recovery count. Before delivery the coordinator freshly rechecks assignment generation/revision, exact Lead/member/runtime parent identity, task state, pause, health revision, acknowledgement and provider operation progress, including rechecking after awaited persistence inspection. Unknown/unavailable ownership is not actionable.
...
Aggregate shutdown checkpoint `b759ae1cda23490285f342d97fef6b1f0797333b` passed [exact-commit GitHub CI](https://github.com/Silktex/dsh-gasteam/actions/runs/33997994650). Receipt `/tmp/gasteam-ci-b759ae1.json`; log `/tmp/gasteam-ci-b759ae1.log`.


## 2026-09-05 — Cold coordinator provisioning and mailbox replay; M2 audit

A new compiled-process scenario wraps the actual reserved teammate provision call after it creates a durable active member, live worker and Git worktree, but before returning to assignment activation. The crash barrier reads the real assignment journal: exact phase `reserved`, no activation receipt, one blocked worker request. After SIGKILL, a fresh unopened coordinator preserves attempt/runtime/worker/generation/worktree identity and completes with one recovery worker request. A separate scenario accurately tests after-admission queued mailbox replay: the stable message ID has one durable queue event and one recipient delivery after another cold restart.

Frozen b759ae1 archive `/tmp/gasteam-m2-provisioning-final-1788650281`, exact two-path `M2-PROVISIONING-OVERLAY.sha256`, passes all21 cold-start acceptance scenarios (`/tmp/gasteam-m2-provisioning-cold-start.log`), typecheck and diff check. These close the final M2 provisioning/mail evidence gap. The earlier requirement audit mapped discovery, explicit authority/single ownership, durable pause and integration/promotion replay to published tests; b759ae1 supplies the aggregate close deadline with retained ownership and passed exact CI. M2's five requirements are now evidenced. Generic scheduler retry policy, full recovery stages and every other open milestone/final acceptance retain their scope.


## 2026-09-05 — Real authenticated Codex verified integration

Actual Codex conformance exposed a sandbox capability gap: a linked worktree needs its canonical shared Git metadata directory to perform ordinary Git commits. The fixed launch pins precisely that verified directory in durable worktree/runtime intent, CLI `--add-dir` arguments and the immutable supervisor manifest. Real-Git tests validate canonical metadata binding and reject swapped grant/receipt state. Sandbox policy is retained; arbitrary prompt-supplied directories are not granted.

The disposable actual-account `gpt-5.4-mini` run completed one file/commit through the real coordinator, external helper, source submission, verification and merge. Evidence `/tmp/gasteam-real-codex-ZH3mtR/conformance-evidence.json` binds attempt1/generation1 to accepted submission and merged integration, exact external owner branch/base, terminal spool SHA/birth identities and exit0. Disposable merged commit `a2cccb6e5c61b26cee56142e260f5af0dd0d72eb`; stdout6869 bytes, stderr0. Provider reported input81199/output1384 tokens; no price estimate. Prior unsupported-model, account-limit and sandbox-workaround failures are retained separately and are not success evidence. User repositories/data/services were not modified by these fixtures.

The opt-in portable fixture is `packages/agent-team/tests/fixtures/real-codex-conformance.mts`; documented invocation uses an advertised account-supported model and bounded execution. Frozen b759ae1 archive `/tmp/gasteam-real-codex-grant-b759-ViHSfN`, authoritative eight-path `real-codex-grant-b759.sha256`, passes install/build/types, compiled focused19, docs and smoke. The unchanged grant behavior's earlier clean00f4c80 archive passes regular409 (two explicit skips) and acceptance19. Logs `/tmp/gasteam-real-codex-grant-b759-*.log`. Real authenticated cancellation/restart, first-class provider usage projection and complete M9/M8 acceptance remain open.


## 2026-09-05 — Published CI and accidental checkout deletion

Published checkpoint `6bef1a2e201f3e19b62981ce02dc400c1a97497b` passed [exact-commit CI](https://github.com/Silktex/dsh-gasteam/actions/runs/33998413601); root rechecked `gh run view 33998413601 --repo Silktex/dsh-gasteam --json headSha,status,conclusion,url` and observed `completed`, `success` and the exact SHA. The watch log remains `/tmp/gasteam-ci-6bef1a2.log`.

The checkout disappeared and was restored at that published commit. The user confirmed accidental deletion. New uncommitted retry/pagination/cancellation work was lost; the agents preserved design and reconstruction notes under `/tmp/gasteam-{m3,m8,m9}-recovery-notes.md`. Prior local test reports for lost changes do not verify their reconstruction. Work resumed in isolated checkouts, with new tests and review required before integration. The unrelated current documentation drafts remain untouched. No milestone is newly marked complete by this recovery record.

A surviving actual Codex restart/cancel run records one external launch intent, the same helper identity after coordinator restart, terminal stop evidence bound to pre-restart ownership, PID-namespace containment and capped spool output (4,331 bytes stdout, zero stderr). Evidence is `/tmp/gasteam-real-codex-restart-cancel-vJKz4L/restart-cancel-evidence.json`. Root inspected this record; the lost portable fixture must be reconstructed before claiming reproducibility from the current checkout. Cancelled-run usage and cost remain unknown.


## 2026-09-05 — Reconstructed configurable assignment retry policy

The first reservation now pins `execution.retryPolicy` (`maxAttempts`, `initialDelayMs`, `multiplier`, `maxDelayMs`); replacement attempts inherit it. `maxAttempts` counts recovery deliveries after initial generation. Legacy reservations retain the published three-delivery 1/2/4-second schedule. A fake-clock real DSH regression reopens the assignment journal, preserves the reserved deadline and proves no early wake followed by exactly one message at the deadline. Zero initial delay remains valid even when exponentiation would overflow.

Health nudges now have separate assignment receipts and counters. Published health nudges previously used generic runtime recovery events; upgrade classifies only exact attempt/generation/message identities bound by the health ledger. It reconstructs effective historical deliveries, deduplicates replayed message IDs and appends an attribution receipt without rewriting old evidence. Tests cover mixed histories, multiple health messages and an actual pending HealthRecoveryStore request with already-delivered Team mailbox message: upgrade/replay preserves one delivery. Root rejected and corrected an initial migration that discarded earlier runtime counts, then a raw-event count that included duplicate replay receipts. Failed migration reads close acquired journal ownership.

Final isolated source `e9c24227001a05f3b7a307c34c5cd33c9fc7dede` in `/tmp/gasteam-m3-retry` passes `pnpm build`, `pnpm typecheck`, `pnpm test` (423 passed, two explicit skips), `pnpm test:acceptance` (21 passed), `pnpm check:docs`, `pnpm test:smoke` and diff checks. Logs `/tmp/gasteam-m3-retry-{build,typecheck-full,test-full,acceptance,docs,smoke}.log`. Root verified all eleven staged source paths have identical Git blob IDs to that tested commit. This closes the retry-policy slice; full M3 completion still requires the remaining requirement audit, and all other unfinished milestones remain in scope.


Retry checkpoint `19bdbd0aa9622baa111c5de42c729b758685b825` passed [exact-commit CI](https://github.com/Silktex/dsh-gasteam/actions/runs/33999977657). Receipt `/tmp/gasteam-ci-19bdbd0.json`.

## 2026-09-05 — External usage receipts and reconstructed restart/cancel harness

External runtime state now retains provider-reported safe integer token counts with an immutable receipt revision bound to its attempt and generation. The observer verifies supervisor terminal/spool ownership before parsing completed-turn JSONL usage. Missing or invalid fields stay unknown; costs are not estimated. Cancelled or uncertain attempts reject first usage reports. Exact replay preserves the original receipt, including across reopen; conflicting usage rejects. Concurrent identical cancellation requests serialize to one journal intent, and close drains pending receipt writes.

The reconstructed opt-in restart/cancel harness passed end to end against the deterministic CLI fixture: ordinary tracked marker commit, owned coordinator-child SIGKILL, a fresh coordinator recovering the same attempt/helper/wrapper birth identities, one launch and cancellation intent, and terminal namespace/spool proof. Root reviewed corrections to child creation, bounded barriers, exit-listener ordering and assertions. New evidence `/tmp/gasteam-real-codex-restart-cancel-SkAXry/restart-cancel-evidence.json`; log `/tmp/gasteam-m9-restart-cancel-final.log`. The earlier authenticated `/tmp/gasteam-real-codex-restart-cancel-vJKz4L` evidence remains tied to its earlier source; it does not prove a paid run of the reconstructed fixture. The exact deterministic invocation is documented in `docs/usage.md`.

Final isolated source `80c6cbb923377b87b6e34458f6a30951ee2fec16` in `/tmp/gasteam-m9` passes build, types, regular419 with two explicit skips, acceptance21, docs and smoke (`/tmp/gasteam-m9-full-*.log`). The final documentation relocation was separately checked. Root verified all seven staged paths against the reviewed Git objects; combined with19bdbd0, build/typecheck and external runtime/assignment/coordinator focused tests pass23 with one optional probe skipped (`/tmp/gasteam-m9-combined-*.log`). Usage remains in the external runtime store; first-class assignment/dashboard projection, complete provider capability coverage and all remaining M9/final acceptance requirements remain open.


External runtime checkpoint `eca8962a202e8d6d9786e7d67bbb68b00d678285` passed [exact-commit CI](https://github.com/Silktex/dsh-gasteam/actions/runs/34000085558). Receipt `/tmp/gasteam-ci-eca8962.json`.

## 2026-09-05 — Workspace history pagination and shutdown ownership

The operator-only typed page Remote and English/Chinese Previous/Next/restart controls now expose readable collection history beyond the bounded overview. A real coordinator Remote regression reaches the 513th attempt; helper and UI regressions cover live changes, foreign/evicted cursors and A→B→A late responses. Snapshots bind caller, collection and revision with process-local signed cursors. Retained RAM is bounded by snapshot count, rows and bytes; oversized collections use private chunk files, with at most two chunks read per page. Initial capture still materializes the coordinator projection. This does not implement the remaining activity feed or operational controls, and no new real-service browser demonstration is claimed.

Review corrected whole-view truncation hiding later records, weakened overview wire limits, whole-file reads on continuation, and leaked files on failed writes. Root added a red coordinator regression proving that cleanup outside retained shutdown released ownership prematurely, then moved asynchronous snapshot cleanup inside that retained operation. The corrected test proves timeout retains ownership, completion joins once, and reopening succeeds. Logs `/tmp/gasteam-m8-shutdown-{red,green,build,types}.log`; corrected focused50 tests pass.

Isolated `/tmp/gasteam-m8` validation logs `.validation-m8/` show build/types, acceptance21, docs and smoke passing. Its initial regular run passed421 and failed one obsolete expected Remote-method list; that list was corrected and the targeted built-library/pagination checks passed. Final isolated source includes root shutdown correction `f17c37c`. On integration with eca8962, fourteen source blobs match the isolated commit exactly and coordinator source/tests merge the published retry changes. Combined build/types and67 focused coordinator/pagination/client/Remote tests pass (`/tmp/gasteam-m8-combined-*.log`). Exact published CI remains the final combined gate.


Workspace history checkpoint `cba0e43183122f635ae6823739a24d7e34c71fe9` passed [exact-commit CI](https://github.com/Silktex/dsh-gasteam/actions/runs/34000465944). Receipt `/tmp/gasteam-ci-cba0e43.json`.

## 2026-09-05 — Provider usage attributed to assignment records

Assignment records now expose optional `externalUsage`, binding reported counts to provider, attempt, generation and runtime receipt revision. Zero counts remain zero, missing counts remain absent, and no costs are inferred. First receipts are fenced after cancellation/termination; an exact existing receipt can replay after reopen. The adapter checks runtime/assignment identity and projects only during reserved/active phases. A cancellation race preserves the underlying external receipt but retires the assignment without a late result or new usage projection; root caught and corrected an early version where the new fence interrupted cancellation reconciliation.

Frozen isolated `6e53ee1ca62d961b4e85cc31c5778344c5309d4d` in `/tmp/gasteam-m9-usage-projection` passes build/types, regular431 with two explicit skips, acceptance21, docs and smoke (`/tmp/m9-projection-final-*.log`). Root inspected the original completed regular log and the agent confirmed its original process exited, without starting a duplicate run. All four integrated source blobs exactly match that commit; combined with cba0e43, build/types and61 coordinator/adapter tests pass (`/tmp/gasteam-usage-combined-*.log`). The dashboard does not yet render these counts. M9 still needs a shared contract for actual routed DSH/external operations and consistent capability reporting; the existing project-level route satisfies the project-or-task routing requirement.


Assignment usage checkpoint `4b1279f8fce29c0d971a51a4b6d367e958ca7c66` passed [exact-commit CI](https://github.com/Silktex/dsh-gasteam/actions/runs/34000620459). Receipt `/tmp/gasteam-ci-4b1279f.json`.

## 2026-09-05 — Provisioning retries and M3 completion audit

Classified transient DSH provisioning failures now become durable terminal history only after a positive owned drain and runtime absence. The receipt accurately says `stopped`: a failed spawn may have partially started. Unknown ownership retains its reservation; authentication, invalid policy and unsupported outcomes do not auto-retry. New generations use a separate provisioning lineage budget and persisted progressive deadline under the already-pinned retry policy. `maxAttempts: 1` permits one replacement after the initial failed provision. Root corrected initial off-by-one exhaustion and backoff that incorrectly reset for every new reservation. Exhaustion remains an actionable recovery-required blocker.

M3 requirement audit: `dispatch-queue.ts` and `scheduler.spec.ts` cover durable priority/order and fair project turns; assignment and coordinator tests cover global/project capacity and pacing, including concurrent `reconcile()` calls. Runtime/provisioning tests prove pinned budgets, reopen-preserved deadlines, uncertainty retention and capacity release after stopped evidence. Coordinator tool/Remote tests capture scheduling inputs/results and verify authority, revision conflicts, pause and queued/active cancellation. The real-Git diamond and cold-process DAG prove automatic verified dependency release. The strengthened failed-verification regression keeps its dependent undispatched while independent report work progresses. These cover all five M3 requirements and its named evidence; M3 is complete without claiming complete recovery or release acceptance.

Isolated source `fe57db5425b76f95670adbea64888205dbbb4661` plus final test/docs delta `257101a61833cf70a706ca0fe45516cce5df7375` passes build/types, regular429 with two explicit skips, docs and smoke; logs `/dev/shm/gasteam-m3-retry-logs/{build,typecheck,full-serial-test,docs,smoke}.log`. The agent's interrupted acceptance invocation is not evidence. Root resolved the additive usage/provisioning field conflict, retained both event families, and validated the combined tree: build/types,178 assignment/Team/coordinator/external-adapter tests, five final transcript/prerequisite tests and all21 actual cold-process acceptance scenarios. Logs `/tmp/gasteam-provision-combined-{build,types,tests,acceptance,docs}.log` and `/tmp/gasteam-provision-final-evidence-tests.log`.


M3 completion checkpoint `d97904931c1fcc6054c8f9dafd5dc254b73c2403` passed [exact-commit CI](https://github.com/Silktex/dsh-gasteam/actions/runs/34001120632). Receipt `/tmp/gasteam-ci-d979049.json`.

## 2026-09-05 — Shared routed-provider lifecycle capabilities

Coordinator DSH and external start/observe/cancel operations now pass through the same routed lifecycle interface and reuse `RuntimeProviderCapabilities` and its capability guard. `CoordinatorView.runtimeCapabilities` exposes enabled routes; disabled execution reports unavailable operations. The routed lifecycle explicitly distinguishes unsupported direct resume/message operations from host-controlled DSH continuation and the separate Codex command builder's resume support. Artifacts remain host-mediated Team/Git submissions. No existing ownership, stop, worktree or admission guarantees are replaced by the wrappers.

Isolated final `2b368487da4292a74177d6c4a07854f36f9ee86d` passes build/types, regular433 with two skips, acceptance21, docs and smoke (`/tmp/m9-contract-full-*.log`). Actual coordinator tests exercise the DSH route and exposed metadata; existing external coordinator tests exercise the routed external lifecycle. Root resolved the dashboard/provider import conflict and verified combined build/types plus54 coordinator/external-routing/contract tests with one optional probe skipped (`/tmp/gasteam-contract-combined-*.log`). This is the common lifecycle foundation, not a claim that M6 handoff or every remaining M9 acceptance item is complete.


## 2026-09-05 — Dashboard reported usage and deterministic disposal test

Workspace overview and paginated attempts now render reported external input, cached input, output and reasoning token counts with English/Chinese labels. Exact zero is preserved; absent counts and cost remain unknown. Receipt identity is checked against the attempt/generation and foreign fields are stripped. Client tests cover reported/unknown usage and stale session responses. Frozen UI source `1205d3dad56b3c32e55a7f06693a94d23d233fea` was integrated with the routed capabilities checkpoint; combined build/types and18 focused tests pass, with one optional probe skipped (`/tmp/gasteam-ui-usage-combined-*.log`).

Shared-capability checkpoint `e010650eb3f362d2828e22a05c9a97679702833b` failed [CI](https://github.com/Silktex/dsh-gasteam/actions/runs/34001254442):447 tests passed, one mailbox disposal test failed and two were skipped. Its second25ms close used real timers under load. The correction keeps both closes under the fake clock and attaches the rejection expectation before advancing it, preserving all ownership and eventual-release assertions. Combined Team tests pass94 (`/tmp/gasteam-mailbox-clock-combined.log`); this is local corrective evidence, not a green result for the failed commit.


Dashboard checkpoint `928d72803e2931d5c87707ba2955e8ad9f3dbba7` passed [exact-commit CI](https://github.com/Silktex/dsh-gasteam/actions/runs/34001873807). Receipt `/var/tmp/gasteam-ci-928d728.json`.

## 2026-09-05 — Disposable demo setup incident

The demo setup continued after `/tmp` temporary-directory creation failed from inode exhaustion. An empty `DSH_HOME` let profile installation rewrite the existing `/home/dsh/.dsh/profiles/web` package manifest, lock metadata and five Team links to the agent's temporary checkout. No user service was started or restarted. Available older backups contain different unrelated plugins, so they were not restored over the profile. Root backed up the affected current files and original link targets under `/var/tmp/gasteam-profile-link-correction-76burcqj`, then changed only those five temporary package paths in three metadata files and the five symlinks to the restored main checkout. Other file content was preserved. This corrects the temporary references; it does not prove exact restoration of the prior profile. The demo resumed in a verified disposable `/var/tmp` home, and the installer now rejects explicitly empty or whitespace-only `DSH_HOME` before invoking the runtime. Agent guard commit `801d94dbf0dbfaa5021699c968ef6b19c1716161` was verified with the user manifest unchanged; root reviewed and integrated it.


## 2026-09-05 — Checkpointed DSH handoff and fenced replacement

After authorized health nudges are exhausted, the exact registered Lead can request handoff through the revision-fenced scheduling Remote/model tool. A separately pinned lineage budget defaults to one. The immutable pre-stop intent binds replacement worker/runtime IDs, checkpoint and a retained predecessor worktree artifact/prompt reference. Positive stopped evidence is required before the new generation reserves; a replay consumes the same intent. Dirty/untracked output remains in the predecessor checkout.

Root review found and corrected a repair-composition bug that stopped an active repair worker but could not reserve its successor. Handoff now carries the repair binding unchanged without spending/resetting its repair round; the next genuine repair still advances normally. The store independently validates the next handoff round, preventing a repeated round-one budget bypass. Tests cover intent-before-stop and stopped-before-reserve reopen, stale predecessor tokens, one replacement, exhaustion, unchanged predecessor checkpoint, real dirty worktree preservation and replacement prompt context. This is not a claim that an OS crash at every handoff boundary has been separately exercised.

Frozen source `953696165cfa95608708e185beb1f808326a43cf` plus reviewed delta `d9d477dd87411444f8b8cb011547ba89878c0d72` is integrated. The initial source passed build/types,448 regular tests with two skips and21 acceptance tests (`/tmp/gasteam-m6-handoff-*.log`); the correction passed24 assignment and96 Team tests. Root verified five unchanged source/test blobs and combined build/types,169 focused tests, all21 cold-process acceptance scenarios and docs checks (`/var/tmp/gasteam-handoff-combined-*.log`). The later escalation-resolution delta is withheld pending correction of premature resolution for an unstarted successor. M6 remains in progress.


Handoff checkpoint `59ad560da179b982cc6871e50fffc563e07da34b` passed [exact-commit CI](https://github.com/Silktex/dsh-gasteam/actions/runs/34002213793). Receipt `/var/tmp/gasteam-ci-59ad560.json`.

## 2026-09-05 — Safe handoff resolution and M6 completion audit

An acknowledged stale predecessor now receives a durable `handoff-replaced` resolution naming its successor only after the successor is active or positively completed. Reserved, uncertain and failed successors leave the old incident visible. Intentional handoff predecessors retain their history and are excluded from new failure classification. Root rejected the earlier unconditional resolution and required the actual coordinator flow in addition to mocked startup outcomes.

The real Team/worktree flow seeds exhausted health state, acknowledges it, calls the revision-fenced coordinator handoff, reconciles repeatedly and closes/reopens the coordinator. It proves one resolved predecessor incident without a bogus failed incident, and independent successor escalation. Health-store tests independently prove durable resolution and generation-scoped re-escalation. This is a coordinator reopen flow with controlled workers, not a separate cold-OS-crash claim. Final isolated source `964877865a7ee92bab23ef8a2ed4cb8486755bd6` passes build/types/docs,454 regular tests with two optional skips and21 cold-process acceptance scenarios (`/tmp/gasteam-m6-resolution-*.log`). Root verified these completed logs and combined build/types,60 focused health/coordinator tests and the additional real flow (`/var/tmp/gasteam-resolution-combined-*.log`).

M6 requirement audit: `health.ts`/`health.spec.ts` cover all classifications, provider deadlines, durable diagnostics/severity/work, bounded repeat escalation, acknowledgement and resolution. `health-runtime-observation.ts` and its tests bind actual DSH operations to identities; the coordinator combines these with durable dependency, non-code acceptance, cancellation and integration states, and supervised external ownership. Existing coordinator and `health-recovery.integration.spec.ts` tests prove legitimate long operations, pause/unavailability fences, durable nudge budgets and one delivery across restart. Assignment/Team tests prove positive stop before replacement, immutable checkpoint/worktree preservation, repair-budget composition and stale-token rejection. The typed operator inbox and tools expose acknowledgement and replacement resolution. No external transport is implemented or enabled; no notification is sent to real people. All five M6 criteria and its named operator flow have evidence, so M6 is complete. Dashboard operational controls and remaining provider/release acceptance retain their own open milestones.


M6 completion checkpoint `0fd32544754b9865f1e7a0f947d28f186eabeee9` passed [exact-commit CI](https://github.com/Silktex/dsh-gasteam/actions/runs/34002569089). Receipt `/var/tmp/gasteam-ci-0fd3254.json`.

## 2026-09-05 — Ordered merge batching and M10 completion

Optional `execution.integrationBatching: { mode: 'ordered', maxCandidates, maxSplitAttempts }` verifies compatible ordered stacks under canonical repository/target ownership; omission preserves sequential mode. Explicit submission prerequisites pin exact source/work identities. Changed stacks and moved targets require fresh verification; bounded splitting admits independent passing changes while excluding failed prerequisites and their dependents. Active registry membership, immutable batch policy, composition budget and pre-promotion receipts survive restart. Individual job selection is exact; task acceptance still uses the existing integration receipts. Review-gated jobs retain their sequential authorization path.

Root review rejected early revisions with missing coordinator dependency mapping, pending prerequisite exclusion, mismatched submission/integration IDs, queue-head receipt selection, reconstructed membership that reset budgets/starved later work, and repeated abandonment of already-failed members during replay. Final tests exercise the shipped Coordinator with more work than the candidate limit, failed prerequisite/dependent plus independent good work at limits one and three, crash/reopen after first actual submission acceptance under changed configuration, pre-registration failures and rejected-predecessor replay. Direct real-Git tests additionally prove good dependent stacks, combined-only failures, target movement, ancestry requirements and idempotent promotion after a prepared receipt. Assertions cover target files/reflogs and per-task states, not just command counts.

Final isolated `98cf759e89982b4373b44e4377b85e7503fc2bc8` in `/var/tmp/gasteam-m10` passes build/types,467 regular tests with two optional skips,21 acceptance tests, docs and standalone smoke. Logs `/var/tmp/gasteam-m10-98cf759-{build,typecheck,test,acceptance,docs,smoke}.log`; the earlier shared-dependency full run is not valid evidence. Root integrated the exact e010650-to-final delta, resolved only the additive usage documentation conflict, and verified combined build/types and78 coordinator/Git/batch/registry/submission tests (`/var/tmp/gasteam-m10-combined-*.log`). All four M10 requirements and five named failure cases now have evidence, so M10 is complete.

Limits: batch composition worktrees and journals are retained; normal candidate retention does not prune them. A crash before registry admission can leave an unpromoted orphan without live ownership. Repeated admission failures in one process reuse the same journal. Individual accepted jobs currently repeat verification after batch promotion; a later check/receipt failure is a durable blocker even though code has landed. The usage guide distinguishes this from pre-promotion rejection. Removing batching is a mode change: finish active batches before switching to sequential execution.


## 2026-09-05 — Integrated dashboard and external startup acceptance

The M10 checkpoint `1045d4a7b399eb55d3bd181fef955a675c1c2fed` passed [exact-commit CI](https://github.com/Silktex/dsh-gasteam/actions/runs/34003184225).

The dashboard now includes bounded journal activity with durable references, signed byte-offset cursors, reconnect conflicts and explicit truncated-tail/per-source-order notices. Review corrected UTF-8 offsets, oversized-row stalls, source starvation, cursor recovery, polling overlap and session-switch races. Project pause/resume, selected-attempt cancellation, due provisioning retry, checkpointed DSH handoff and escalation acknowledgement use existing typed authority and revision fences. Cancellation checks the selected attempt generation before changing its dispatch request. Retry retains the original budget/provider/checkpoint and rechecks task readiness, workspace dependencies, capacity and pacing. Merge-batch registry membership and provisioning lineage join queue age, last progress, capacity and reported usage diagnostics; absent usage/timestamps remain unknown.

Frozen dashboard deltas end at `c28844fa0aa56df2488b9dcf53161bfcec62b9e3`, `8ed87cae92830ea00a3547b5b8d108f340efb095`, `7415bf13b53a74bac6f7d868822f0ed7c527cd7d` and `34ab0245eeacd5903b023765a139d730e1b42d11`. Host/client tests cover selected identity, stale replacement tokens, reopened prerequisites, retry budgets, keyboard actions, reconnect and unknown/zero/nonzero usage.

A real Web demo uses the shipped coordinator and typed Remote stack with a controlled provider and 130 retained attempts; it makes no paid calls. Real browsing exposed and fixed automatic JSX runtime, multi-module CSS injection and root-context coordinator lookup. The launcher validates a disposable home before profile installation and accepts the documented pnpm argument separator. Chrome 145 at 1600×1100 verified 128 activity rows, trusted keyboard Pause/Resume, 64-row history Next/Previous, observer rejection and Lead recovery without console errors. Root inspected the screenshots and sanitized transcript: [browser evidence](evidence/dashboard/browser.json), [activity screenshot](evidence/dashboard/activity.png), [operation screenshot](evidence/dashboard/controls.png). The last read-only merge-batch/retry diagnostics were added after this browser run and are covered by host/client tests. [Demo instructions](dashboard-demo.md) reproduce the real service. Earlier loading-only screenshots are not acceptance evidence.

External startup acceptance ends at `66e5d86da61173a600d09e3195ca674958fd7ae9`. Known executable launch failures receive bounded provisioning retries only with positive stopped evidence and no started turn/thread; structured authentication errors are nonretryable and store category/exit diagnostics without credentials. Cancellation wins over concurrent startup failure classification. Real coordinator tests remove an executable after admission, reopen with the same intent and deadline, restore it and prove exactly one successful replacement. Unknown/runtime failures remain failures; no DSH fallback is introduced. Existing actual authenticated Codex 0.153.4 task and restart/cancel artifacts remain the real-provider evidence recorded above; deterministic fixtures are not relabeled as new paid runs.

Combined main build and typecheck pass, and the full regular suite passes 504 tests with two optional probes skipped (`/var/tmp/gasteam-final-{build,types,tests}.log`). An earlier combined run failed only two obsolete Remote method inventory expectations; both were corrected and their ten focused tests passed before this green full run. Missing upstream source-map warnings remain non-fatal. Final release additions and exact published CI are recorded separately below.

On 2026-09-05, the official [OpenAI noninteractive CLI documentation](https://learn.chatgpt.com/docs/non-interactive-mode) was checked against the adapter: `exec --json` emits JSONL with thread/turn/item/error events and reported usage, and `exec resume <SESSION_ID>` supports exact-session continuation. The official [sandbox documentation](https://learn.chatgpt.com/docs/sandboxing) documents the configured sandbox modes and distinguishes approval from isolation. Current pages do not establish historical 0.153.4 flag ordering or `--add-dir`/stdin/cwd details; those are version-specific conformance and installed-runtime evidence, not claims inferred from current documentation.

The combined 21 fresh-process acceptance scenarios and installed CLI smoke also pass (`/var/tmp/gasteam-final-{acceptance,smoke}.log`).


## 2026-09-05 — Standalone release and final acceptance mapping

The integrated release gate passes at `/var/tmp/gasteam-standalone-release-zuNVLS`; full log `/var/tmp/gasteam-final-release.log`. It builds and packs all five plugins, installs published DSH plus the two committed patches in an unrelated consumer, repeats a frozen install, checks package/browser resolution and patch contents, composes Web and autonomous profiles, runs an installed headless CLI worker through actual Git commit/verification/promotion/release, and completes a four-task DAG after killing and restarting the coordinator. Legacy assignment and batch fixtures are backed up, opened/advanced with the packed current implementation, and restored from the untouched backup. No full-Harness source checkout or development package link is used.

The actual failed-verification CLI path exposed an omitted optional `failureKind` in the strict model-tool result schema. The correction and a real Git failing-verifier tool regression pass all 17 tool tests and typecheck on the combined tree (`/var/tmp/gasteam-final-release-{tool,types}.log`). Final release delta is `c1af2160..9b082bb29ae6fb1b5003961152b57395ae5f3825`; it adds the packed gate to fresh-clone CI. The earlier full combined suite passed 504 tests with two optional skips; the final schema regression adds one test. Exact published CI below covers the entire final composition.

| Final acceptance scenario | Evidence on shipped services |
| --- | --- |
| Unattended DAG; browser closed; cold server restart | `tests/cold-start.acceptance.ts`: timer-driven diamond, unopened project discovery, durable admission replay; independent packed four-task restart gate |
| Crash during submission/promotion | Fresh-process accepted submission/verified-candidate barriers and idempotent promotion; integration restart and real-Git tests |
| Worker turnover | Team test completes nine actual sequential workers with capacity two; concurrent admission tests preserve the bound |
| Stale worker | Assignment/Team generation and revision fences reject superseded completion, submission and reclaim |
| Verification failure; target movement | Real Git failing verification, bounded repair, pinned-source fresh candidates, review receipts and retained prior checkouts |
| Workflow handoff | Workflow intent/checkpoint tests and fresh-process report/code/review resumption |
| Recovery exhausted | Actual Team handoff/acknowledgement/reconcile/reopen flow resolves only a safely started successor; new generation escalates independently |
| Two repositories | Cold-process cross-project dependency and independent repository failure scenarios |
| Merge batch failure | Actual coordinator plus real-Git ordered stack tests, failed prerequisite isolation, composition budgets and restart-safe membership |
| External runtime | Previously recorded authenticated Codex verified commit plus actual restart/cancel receipt; deterministic current parser/ownership/routing/startup/cancellation suites |
| Output preservation | Git retention tests preserve dirty, ignored, untracked, unmerged, uncertain and owned work; handoff retains old checkout |
| Standalone installation | Current unrelated packed consumer/profile CLI and coordinator restart gate, plus legacy fixture upgrade/restore |

Command validation distinguishes execution from syntax checking. Build/types/tests/process acceptance/smoke/release, profile installation/composition/removal, deterministic external restart/cancel and diagnostic commands execute in disposable environments. Backup/archive/restore examples use substituted disposable absolute paths. Real authenticated external behavior has its separate recorded receipts. `check:docs` checks links and shell syntax only. Production systemd/restart/user rollback commands are conditional operator procedures, not actions authorized by this request; M11's conditional localhost-install clause is not invoked. The five private plugins have no executed registry installation path; supported standalone installation uses tarballs. No registry availability or production service migration is claimed.

The executed documented-command transcript is `/var/tmp/gasteam-m11-documented-commands.log` (final marker `DOCUMENTED_COMMAND_AUDIT=passed`), with disposable profiles and both backup archives retained in `/var/tmp/gasteam-doc-commands-SzpLQZ`. It includes profile Web/headless installation, five-package removal, versions/status/diff, five persistence tests, tar archive inspection and deterministic external restart/cancel at `/var/tmp/gasteam-real-codex-restart-cancel-ES8CH3`. Root checked the completed transcript marker. These are disposable substitutions, not a claim that production paths or systemd were modified.


Publication checkpoint `19cf882dfbc0e12eafaac2234fb3c65a79d295e2` was pushed, but [its exact CI](https://github.com/Silktex/dsh-gasteam/actions/runs/34005229464) failed the external startup-retry scenario (504 other tests passed, two skipped). It is not a green release receipt. Investigation found that the test opened a coordinator containing ready work before removing its executable, allowing the initial scan to launch it first. The correction and replacement CI result are recorded next.

Disposable browser services on 49184/49185 and CDP 9224 are closed; their exact isolated homes were checked and prior PID 1670460 had exited. No unrelated process was signaled. Retained browser evidence and fixture data remain available.


## 2026-09-05 — Fast external launch output preservation

The deterministic deleted-executable case exposed a product race in addition to the test ordering issue: supervisor stdout/stderr listeners were attached only after asynchronous process-identity inspection and durable writes. A fast namespace-wrapper exec failure could exit and lose its error stream first. The supervisor now attaches bounded capture immediately after spawn. Terminal/stop receipts await queued output writes; uncertain launch cleanup waits for process closure and then the latest queued writes before releasing its lock. Rejections remain observable through drain while avoiding an unhandled-rejection window.

The test now deletes the actual executable during its successful admission/authentication probe, making deletion happen before target launch. It controls scheduler time immediately below and at the persisted retry deadline, while real subprocess timers remain operational. No synthetic ENOENT output or later-filesystem-absence inference is used. The existing startup classifier is unchanged. A new supervisor regression checks actual missing-executable stderr; overflow/cancellation bounds remain covered. Frozen `3df0c9e69f43dc63c41db48ceebb9e833f568382` passes build/types, 37 focused tests with one compiled-route skip, three genuine launch/retry runs, and 506 regular tests with two optional skips. Root reviewed the exact three-file delta and its drain ordering before integrating.


The output-capture checkpoint `80d60014a80c85719b6fc7cb11e9a96a8d8c069b` passed local build and 18 combined supervisor/coordinator checks with one optional compiled-route skip, but [exact CI](https://github.com/Silktex/dsh-gasteam/actions/runs/34005782618) exposed a second fast-exit boundary: asynchronous birth-identity reading can yield until after the wrapper is reaped, leaving no process identity and therefore no admissible terminal receipt. CI reported 505 passing tests, one failure and two skips. This checkpoint is also not green release evidence. The follow-up captures the real initial process birth identity before yielding; validation follows below.
...
The documentation checkpoint `9f3c5feecbcd7a6eedca309568ea9a3daf21090c` [CI34006204740](https://github.com/Silktex/dsh-gasteam/actions/runs/34006204740) exposed an intermittent overflow/cancellation ordering bug despite the preceding green source run. It reported 505 passing tests, one failure and two skips. An already-exited matching ChildProcess was treated like an unavailable handle, so cancellation omitted its drained exit/spool receipt. A signal is unnecessary when the owned child already exited, but a matching positive close receipt remains required. The final corrective publication must pass CI before closing the goal.


Final overflow/close correction `70aae157e1223067fbb38779c4f41ebcace6dc30` separates a mismatched handle from an already-exited matching child. The latter waits a bounded time for its actual close, then records exit and drained spool with truthful empty signals. The TERM-to-KILL boundary uses the same close fence. A deterministic real-overflow regression forces this ordering; another withholds actual close delivery and requires uncertainty without exit/spool proof. Existing live cancellation retains its SIGTERM assertion. Build/types and 58 focused tests pass with one existing skip.

Root's combined supervisor run then exposed an unhandled stdin EPIPE after early child exit (all assertions passed but Vitest correctly returned failure). Follow-up `f432ba351ee3c3cf803abab6f8c6bcd00db39f4f` installs stdin error handling immediately after spawn. EPIPE cannot crash the helper and is never used as stop evidence. Unexpected input errors produce explicit uncertainty after close/drain and reject completion without publishing terminal receipts; the new EIO regression covers that behavior. The agent's existing 15 supervisor tests and new fault test pass, with build/types green. No classifier or ownership requirement was weakened to obtain these results.


## 2026-09-05 — Corrective publication verified and goal closed
