...
创建具名共享任务持久批次，其进度可在 Lead 重启后保留。仅限 Team Lead。
...
使用当前 revision 编辑或永久归档持久任务批次。仅限 Team Lead。
...
停止重试阻塞的集成，并保留候选 checkout 供检查。新请求会固定新的输入。仅限 Team Lead。
...
将静止 worker 的已提交产物排队，以便独立验证并集成到配置的目标 branch。仅限 Team Lead。
...
验证并推进最早排队的集成，或恢复其已验证的推进。验证失败会保留候选 checkout。仅限 Team Lead。
...
在 commit 合并后释放静止 teammate 的 worktree。脏文件或未合并工作会保留。仅限 Team Lead。
...
工具限定于隐式 Team Lead 和持久 teammate。本目录包含可选 worktree 释放和集成工具；部署通过对应的 Team provider 配置启用它们。已提供的 dsh-base bundle 默认禁用该包；文档中的 Agent Teams profile patch 会启用它，并禁用旧版 continuable-child control 名称。
