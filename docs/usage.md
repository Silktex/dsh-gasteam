...
Set `execution.retryPolicy` to pin bounded provisioning retry timing. Its `maxAttempts` is the number of replacement generations allowed after the initial provision: `maxAttempts: 1` permits one replacement after a classified provision failure. `initialDelayMs`, `multiplier`, and `maxDelayMs` determine the durable replacement deadline. A retryable provision failure records its diagnostic, lineage count, and deadline in the assignment journal; the coordinator does not create the replacement early. Authentication, invalid-policy, unsupported, and unclassified ownership outcomes do not retry automatically. A provider failure releases capacity only after a matching runtime is drained and absent; that receipt is `stopped`, because a rejected spawn may have partially started. Uncertain ownership retains its reservation and capacity for reconciliation.

This provisioning budget is separate from interrupted-runtime recovery deliveries and from health nudges. Interrupted runtime recovery uses the same pinned policy only to bound wakeups for an already active generation; health nudge and integration-repair budgets remain independent. Exhausted or non-retryable provisioning history stays visible as a durable recovery-required blocker and requires an explicit operator recovery path.
...
When the Web profile is installed, the browser panel shows roster, task, batch, integration, recovery, workspace history, live activity, provider usage, and operator controls. Project registration, submission admission, report acceptance, workflow publication authorization, and publication remain on their existing host, model-tool, or trusted-server surfaces.

## Coordinator controls and acceptance

The shipped coordinator Remote operations include Team and workflow mutations plus `workspaceDashboard`, `workspaceDashboardPage`, `workspaceActivityPage`, `scheduling`, `controlScheduling`, `healthInbox`, and `acknowledgeHealth`. `scheduling` takes `{projectId}`. `controlScheduling` takes current project, request, and attempt revisions as required by one action: pause/resume, cancel with a reason, reprioritize, retry a provisioning failure, or hand off an eligible DSH attempt. The model tools expose the matching scheduling operations as `team_dispatch_status`, `team_dispatch_pause`, `team_dispatch_priority`, `team_dispatch_cancel`, `team_dispatch_retry`, and `team_dispatch_handoff` when the coordinator tool plugin is enabled.
...
The host-only `submit` and project `register` operations remain intentionally absent from model tools and the dashboard. In the dashboard, select a project to load its scheduling state and health inbox. Pause/resume affects new dispatch, cancellation requires a nonempty reason, and incident acknowledgement uses the displayed revision. Select an attempt as well to expose its applicable cancellation, provisioning-retry, or DSH-handoff control. Every mutation is checked again by the coordinator against the current revisions and authority. See [the architecture decisions](autonomous-architecture.md) and [completion evidence](completion-evidence.md) for the authority and acceptance boundaries.
...
The Web session header includes the workspace dashboard for the configured operator. It shows projects, attempts, workflow steps, cross-project batches, dispatch blockers, integration results, health incidents, and provider-reported usage. Project/attempt selection filters the view and reveals the eligible operations described above; refresh reloads the authorized snapshot. Disabled retry controls explain the exact gate: the attempt must be a terminal, classified, retryable provisioning failure with a positive stopped-runtime receipt, remaining pinned budget, and an elapsed durable deadline. Handoff is limited to an active DSH attempt after authorized recovery nudges are exhausted and while the separate handoff budget remains. External attempts cannot be handed off.

Collection history uses `agentTeams/workspaceDashboardPage`; the dashboard requests 64 rows and passes the returned cursor for Next. Previous/Next navigate a pinned snapshot, so concurrent changes do not shift rows. If the server restarts or evicts that snapshot, use Restart collection to fetch a fresh page. Activity uses `agentTeams/workspaceActivityPage`, preserves each journal source's sequence, polls forward, and retains the latest 128 rows in the browser. `Time unknown` is valid for records without a durable timestamp, and the history-gap notice means older rows were intentionally not read. Browser proof of the shipped Activity and controls is retained in [the dashboard evidence](evidence/dashboard/browser.json), with [Activity](evidence/dashboard/activity.png) and [controls](evidence/dashboard/controls.png) screenshots.
...
Delivery uses one reserved durable Team message ID; replay cannot consume another recovery count or enqueue the same message twice. Exhaustion leaves the escalation visible and allows other projects to continue.

After the configured DSH nudge budget is exhausted, the registered Lead may use `team_dispatch_handoff` with the current project request revision and exact attempt generation/revision. `execution.maxHandoffs` pins a separate lineage budget (default one) and never spends provisioning, interrupted-runtime, health-nudge, or integration-repair budget. The coordinator first writes one immutable handoff intent containing the replacement IDs and checkpoint, including a file artifact and prompt reference to the old worktree. It then requires a positive `stopped` drain receipt before reserving the fenced replacement. The predecessor worktree is retained, including dirty and untracked output; handoff does not delete it. A restart replays the existing intent and cannot create another replacement or spend another handoff budget. A successful fenced replacement durably resolves only the retired generation's stale inbox item, while the new generation has its own deadline and bounded escalation history. Exhaustion remains visible in assignment history and requires an explicit later operator resolution or re-escalation; this stage sends no external notification and does not hand off external-provider work.
