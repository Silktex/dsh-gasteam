...
The Web session header now includes a read-only workspace dashboard for the configured operator. It uses `agentTeams/workspaceDashboard` with `{}` and shows projects, attempts, workflow steps, cross-project batches, dispatch blockers, integration results, and health incidents. Project/attempt selection filters the view; refresh reloads the authorized snapshot. Collections show explicit truncation notices. The collection history control uses `agentTeams/workspaceDashboardPage` with `{ "collection": "attempts", "pageSize": 64 }`; pass its `nextCursor` back as `cursor` for continuation. Previous/Next navigate a pinned snapshot, so concurrent changes do not shift rows. If the server restarts or evicts that snapshot, use the displayed restart control to fetch a fresh page. This is a history browser; workspace mutations and a reconnecting activity feed remain unfinished. Batch mutations remain available through the typed Remote/model operations above.
