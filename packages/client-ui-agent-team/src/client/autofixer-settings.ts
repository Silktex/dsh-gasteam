...
export function generateCrontabCommand(config: AutofixerConfig, repoPath = '/path/to/repository'): string {
...
export function generateSystemdService(config: AutofixerConfig, repoPath = '/path/to/repository'): string {
  const flag = config.verificationGates.acceptance ? ' --full' : ''
  return `[Unit]
