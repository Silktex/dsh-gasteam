---
description: "让模型创建、发消息与协调 teammate 的工具，供组合实验性 Team 插件的部署方阅读。"
...
`dsh-experimental-tool-agent-team` 在团队领域包之上给模型一套团队工具：创建具名 teammate、给它们发消息或后续任务、查看谁在线、等待进展、中断卡住的 teammate，以及管理共享任务板以及持久任务批次。每个成员的提示词中都有一段简短策略，教模型何时组建团队（只有你要求时）以及如何在共享工作区协作。挂载它会用同名的团队工具取代旧版 subagent 控件，因此想同时使用两者的组合必须禁用旧定义。它是实验性的：不进入正式发布、不承诺稳定性，并且只有你明确要求组建团队时才会创建 teammate。
...
当模型应该通过工具运行一支团队时，在 `@deepseek-ai/dsh-experimental-agent-team` 之上挂载本包。挂载后，每个团队成员——Lead 与每个 teammate——都会获得相同的 Team 工具，外加一段说明自身角色与名字的策略段落。
...
- **管理任务板**——`team_task_create`、`team_task_list`、`team_task_get` 与 `team_task_update` 添加、浏览、读取与更新共享任务；complete 会记录结果依据，直到任务 reopen。
...
| 文件 | 职责 |
|---|---|
| [`src/index.ts`](src/index.ts) | 插件入口：配置、固定策略文本与scoped 工具注册 |
| [`src/invariant.ts`](src/invariant.ts) | 不变式伴生插件（无运行时不变式；委托只能通过 `ctx.agentTeams` 观察） |

### Worktree 与集成工具

Team service 选择 worktree provider 后，`team_worktree_release` 提供保守清理。选择集成 provider 会启用队列接纳、读取、执行和显式放弃。仅 Lead 可以修改这些资源；验证失败会保留候选 checkout。批次工具创建、读取、编辑和归档具名任务组。[Team service README](../agent-team/README.zh.md)拥有部署与生命周期要求。

### 策略与工具

member scope 上的一个 `team:policy` 段落教每个成员自己的角色与协作规则；固定文本与scoped 工具注册都声明在 [`src/index.ts`](src/index.ts)。工具 schema 只出现在 Team member scope 中，因此非 Team subagent 保持默认目录。与旧全局 continuable-subagent 控件同名的 scoped 注册只会为团队成员覆盖这些全局控件。
...
一段稳定策略会说明确切 Team role／name／id、显式 delegation 要求、配置的共享 checkout 或 worktree 行为、文件 stale-version 恢复、Bash／formatter／codegen 风险、task／write-scope 协调、quiet 与 waking 投递区别、mailbox 不重试规则，以及 Lead 必须在回答前等待。`spawn_teammate`、`team_batch_list` 和其他 Team schema 只出现在 Team member scope。
