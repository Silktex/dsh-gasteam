...
it('bounds every dashboard collection, workflow step, and queue blocker list with explicit truncation', () => {
  const view = projectWorkspaceDashboard(input, { projects: 1, attempts: 1, workflows: 1, workflowSteps: 1, batches: 0, queue: 1, queueBlockers: 0, integrations: 0, escalations: 0 })
  expect(view).toMatchObject({
    projectsTruncated: true, attemptsTruncated: true, workflowsTruncated: false,
    batchesTruncated: true, queueTruncated: false, integrationsTruncated: true, escalationsTruncated: true,
  })
...
it('preserves authoritative uncertain health rather than inventing active progress or terminal success', () => {
  const view = projectWorkspaceDashboard({ ...input, attempts: [input.attempts[1]], workflows: [], batches: [], queue: [], integrations: [], escalations: [] })
