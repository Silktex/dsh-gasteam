...
const intent = z.object({ attemptId: id, generation, provider: id, runtimeIdentity, admission: admission.optional(), inputSha256: z.string().regex(/^[a-f0-9]{64}$/).optional(), spool: spool.optional(), supervision: supervision.optional() }).strict().superRefine((value, ctx) => {
...
function sameIntent(old: Record, value: z.output<typeof intent>): void {
  if (old.provider !== value.provider || JSON.stringify(old.runtimeIdentity) !== JSON.stringify(value.runtimeIdentity) || JSON.stringify(old.admission) !== JSON.stringify(value.admission) || old.inputSha256 !== value.inputSha256 || JSON.stringify(old.spool) !== JSON.stringify(value.spool) || JSON.stringify(old.supervision) !== JSON.stringify(value.supervision)) throw new Error('External runtime launch intent is immutable')
}
